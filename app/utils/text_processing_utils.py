...
def extract_total_amount(text):
    ...
    """
    patterns = [
        r"Total\s*[:\-]?\s*(?:INR|₹)?\s*([\d.,]+)(?!\s*\d)",  # Matches 'Total: 200.00', 'Total - INR 200.00'
        r"Grand\s*Total\s*[:\-]?\s*(?:INR|₹)?\s*([\d.,]+)",  # Matches 'Grand Total: ₹200.00'
        r"Total\s*Amount\s*Paid\s*[:\-]?\s*(?:INR|₹)?\s*([\d.,]+)",  # Matches 'Total Amount Paid: ₹200.00'
        r"PAYMENT SUMMARY.*?\n.*?Credit Card\s*[:\-]?\s*(?:INR|₹)?\s*([\d.,]+)",  # Matches under payment summary
        r"Sub\s*Total\s*[:\-]?\s*(?:INR|₹)?\s*([\d.,]+)",  # Matches 'Sub Total: ₹200.00'
        r"Amount\s*Paid\s*[:\-]?\s*(?:INR|₹)?\s*([\d.,]+)",  # Matches 'Amount Paid: ₹200.00'
        r"Net\s*Payable\s*[:\-]?\s*(?:INR|₹)?\s*([\d.,]+)",  # Matches 'Net Payable: ₹200.00'
        r"Balance\s*Due\s*[:\-]?\s*(?:INR|₹)?\s*([\d.,]+)"  # Matches 'Balance Due: ₹200.00'
    ]
...
def extract_transaction_id(text):
    ...
    patterns = [
        ...
        r"Bill\s*No\.\s*:\s*(\d+)"  # Matches 'Bill No.: 14644' followed by newline or other characters
    ]
